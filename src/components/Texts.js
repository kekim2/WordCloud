...
const databaseURL = "https://word-cloud-9e303.firebaseio.com/"
...
class Texts extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            fileName: '',
            fileContent: null, //문장 입력
            texts: {},
            textName: '', //text ID
            dialog: false //추가창이 열리고 닫히게 할수 있음.
        }
    }

    _get() {
        fetch(`${databaseURL}/texts.json`).then(res => {
            if(res.status != 200 /*port가 연결안되어있을때*/) {
                throw new Error(res.statusText);
            }
            return res.json();
        }).then(words => this.setState({texts: (texts == null ) ? {} : texts}));
    }
    ...
    render() {
        ...
        return (
                        ...
                        <Card key={id}>
                            <CardContent>
                                <Typography color="textSecondary" gutterBottom>
                                    ...
                                    {text.textContent.substring(0, 14) + "..."}
                                    </Typography>
                                        </Grid>      
                                    <Grid item xs={3}>                                              
                                        <Link component={ RouterLink } to={"detail/" + id}>
                                        <Button variant="contained" color="primary">Looking</Button>
                                        </Link>
                                        </Grid>                                         
                                    <Grid item xs={3}>
          ...
          <Dialog open={this.state.dialog} onClose={this.handleDialogToggle}>
              <DialogTitle>Text Addition</DialogTitle>
              <DialogContent>
                ...
                <label htmlFor="raised-button-file">
                    <Button variant="contained" color="primary" component="span" name="file">
                        {this.state.fileName === "" ? ".txt file selection" : this.state.fileName}
                    </Button>
                </label>
                <TextTruncate 
                    line={1}
